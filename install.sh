...
setupConfigurations() {
    ...
    cp -R "$HYPRLAND_DIR/extra/alacritty.toml" "$XDG_CONFIG_HOME/wal/templates" >/dev/null 2>&1 || { printf "%b\n" "${RED}:: Failed to set up alacritty.toml${RC}"; }
...
setupSDDMPfp() {
    ...
    $ESCALATION_TOOL cp "$HYPRLAND_DIR/pfps/broke-mcdonalds-worker.jpg" "/var/lib/AccountsService/icons/$USERNAME"
